...
module Phkit.SoftBound.Checker
...
softBoundAddCheckResultOf modu =
  fwdTransformResultOf modu sbFunctionCheckLattice sbCheckTransfer sbCheckRewrite

sbCheckTransfer :: PhFwdTransfer SbFunctionCheckFact
sbCheckTransfer = CH.mkFTransfer trans
  where
    trans :: PhInstruction e x -> SbFunctionCheckFact -> CH.Fact x SbFunctionCheckFact
    trans NameInsn {} f = f
...
sbCheckRewrite = CH.mkFRewrite (\phI f -> fmap Just $ maybeAddCheck phI f)
...
maybeAddCheck phI@InsnInsn {} f = return $ prependAllChecks (getChecks phI f) phI
maybeAddCheck phI@TermInsn {} f = return $ prependAllChecks (getChecks phI f) phI

prependAllChecks :: [LGA.Instruction]
...
prependAllChecks invokes tail =
  ...
  in foldr folder (phGUnit tail) invokes
...
instance SbCheckable x =>
         SbCheckable (LGAI.Named x) where
  getChecks (LGA.Do it) f = getChecks it f
  getChecks (_ := it) f = getChecks it f
...
instance SbCheckable LGAI.Instruction where
  ...
  getChecks LGAI.Store {LGAI.address = addrOp} f =
    let opChecks = getChecks addrOp f
    in case addrOp of
...
instance SbCheckable LGAI.Terminator
...
instance SbCheckable LGAO.Operand where
  getChecks (LGAO.LocalReference t n) f =
    let pstate = DM.lookup n f
        psize = sizeOfPtrType t
    in case pstate of
         Just (SbChecked ptr_meta size_set _) ->
           if DS.member psize size_set
             then []
             else [ mkSbCheck
                      n
                      ptr_meta
                      (LGAC.Int 8 $ toInteger $ sizeOfPtrType t)
                  ]
         _ -> []
  getChecks _ _ = []
...
sbNameCheckLattice =
  ...
  , CH.fact_join =
    \_ (CH.OldFact oldF) (CH.NewFact newF) -> joinSbNameCheckState oldF newF
  }

joinSbNameCheckState :: SbNameCheckState
                     -> SbNameCheckState
                     -> (CH.ChangeFlag, SbNameCheckState)
joinSbNameCheckState oldF newF =
  let joined = justJoinSbNameCheckState oldF newF
  in (CH.changeIf (joined /= oldF), joined)

justJoinSbNameCheckState :: SbNameCheckState -> SbNameCheckState -> SbNameCheckState
-- Equal, must be the first pattern
justJoinSbNameCheckState f1 f2
  | f1 == f2 = f1
-- SbBottom
