...
maybeAddCheck phI@InsnInsn{} f = 
    return $ prependAllChecks (getChecks phI f) phI
maybeAddCheck phI@TermInsn{} f = 
    return $ prependAllChecks (getChecks phI f) phI
...
prependAllChecks invokes tail = 
    let folder
            ...
            -> CH.Graph PhInstruction CH.O x
        folder invoke currG = 
            CH.catNodeOOGraph (InsnInsn (LGA.Do invoke)) currG
...
justJoinSbNameCheckState :: SbNameCheckState
                         -> SbNameCheckState
                         -> SbNameCheckState
