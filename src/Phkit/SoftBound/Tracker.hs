...
mkSbUpdate :: LGA.Name -- ^ ptr
           -> LGA.Instruction -- ^ ptr_base_exp
           -> LGA.Instruction -- ^ ptr_bound_exp
           -> NameLabelMapFuelM [LGA.Named LGA.Instruction]
           -- ^ generated assignment and call instruction
mkSbUpdate ptr ptr_base_exp ptr_bound_exp = do
  base_name <- freshName
  bound_name <- freshName
  let base_assign = base_name LGA.:= ptr_base_exp
  let base_bound = bound_name LGA.:= ptr_bound_exp
  

mkSbUpdateCall :: LGA.Name -- ^ ptr
               -> LGA.Name -- ^ base name
               -> LGA.Name -- ^ bound name
               -> NameLabelMapFuelM (LGA.Named LGA.Instruction)
mkSbUpdateCall ptr base_name bound_name =
  tracker_name <- freshName
  return (tracker_name LGA.:= LGAI.Call {
             LGAI.tailCallKind = Nothing,
             LGAI.callingConvention = LGACa.C,
             LGAI.returnAttributes = [],
             LGAI.function = LGAO.ConstantOperand LGAO.GlobalReference LGA.Name "sbupdate",
