...
_mkSbCall retT name args =
  ...
  , LGAI.function =
    ...
    LGA.Name name
  , LGAI.arguments = map (\o -> (o, [])) args
  , LGAI.functionAttributes = []
  , LGAI.metadata = []
  }

-- | sbcheck(ptr, ptr_meta, ptr_size)
mkSbCheck :: LGA.Name -> LGA.Name -> LGAC.Constant -> LGA.Instruction
...
maybeGetCheckedPtr :: LGA.Instruction -> Maybe (LGA.Name, LGA.Name, Int)
maybeGetCheckedPtr (LGAI.Call {LGAI.function = Right (LGAO.ConstantOperand (LGAC.GlobalReference _ (LGA.Name "sbcheck")))
                              ,LGAI.arguments = [((LGAO.LocalReference _ arg_ptr), _), ((LGAO.LocalReference _ arg_ptr_meta), _), ((LGAO.ConstantOperand (LGAC.Int _ arg_ptr_size')), _)]}) =
  Just (arg_ptr, arg_ptr_meta, fromInteger arg_ptr_size')
maybeGetCheckedPtr _ = Nothing

-- | sbsave(ptr_ptr, ptr_meta)
mkSbSave :: LGA.Name -> LGA.Name -> LGA.Instruction
...
maybeGetSavedPptr (LGAI.Call {LGAI.function = Right (LGAO.ConstantOperand (LGAC.GlobalReference _ (LGA.Name "sbsave")))
                             ,LGAI.arguments = [((LGAO.LocalReference _ arg_pptr), _), _, _]}) =
  Just arg_pptr
...
-- | ptr_meta = sbcopy(ptr, ptr_meta)
mkSbCopy :: LGA.Name -> LGA.Name -> LGA.Name -> LGA.Named LGA.Instruction
...
mkSbLoad :: LGA.Name -> LGA.Name -> LGA.Name -> LGA.Named LGA.Instruction
...
maybeGetTrackedPtr (meta_name := (LGAI.Call {LGAI.function = Right (LGAO.ConstantOperand (LGAC.GlobalReference _ (LGA.Name "sbload")))
                                            ,LGAI.arguments = [(LGAO.LocalReference _ arg_ptr, _), _]})) =
  Just (arg_ptr, meta_name)
maybeGetTrackedPtr (meta_name := (LGAI.Call {LGAI.function = Right (LGAO.ConstantOperand (LGAC.GlobalReference _ (LGA.Name "sbinit")))
                                            ,LGAI.arguments = [(LGAO.LocalReference _ arg_ptr, _), _]})) =
  Just (arg_ptr, meta_name)
