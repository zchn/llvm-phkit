...
module Phkit.SoftBound.Tracker
  ( softBoundAddTrackResultOf
  ) where
...
softBoundAddTrackResultOf modu =
  bwdTransformResultOf modu sbFunctionTrackLattice sbTrackTransfer sbTrackRewrite
...
sbTrackTransfer = CH.mkBTransfer trans
  where
    trans :: PhInstruction e x -> CH.Fact x SbFunctionTrackFact -> SbFunctionTrackFact
    trans NameInsn {} f = f
    trans (InsnInsn n) f =
      case maybeGetTrackedPtr n of
        Just (ptr, ptr_meta) -> addTrackedPtrToFact ptr ptr_meta f
        Nothing -> f
    trans n@(TermInsn term ls) fbase =
      case ls of
        ...
        -- matter here.
        (h:_) -> CH.joinFacts sbFunctionTrackLattice h $ successorFacts n fbase

sbTrackRewrite :: PhBwdRewrite SbFunctionTrackFact
sbTrackRewrite = CH.mkBRewrite (\phI f -> fmap Just $ maybeAddTrack phI f)
...
maybeAddTrack phI@(InsnInsn (ptr := LGA.Load {LGAI.address = LGAO.LocalReference _ ptr_ptr})) f =
  if DM.findWithDefault SbTBottom ptr f == SbTBottom
    then do
      ptr_meta <- CH.liftFuel freshName
      return $
        (phGUnit phI) `CH.catGraphNodeOO`
        (InsnInsn $ mkSbLoad ptr_meta ptr ptr_ptr)
    else return $ phGUnit phI
maybeAddTrack phI@(InsnInsn (ptr := LGA.Alloca {LGA.allocatedType = ty
                                               ,LGA.numElements = maybeC
                                               ,LGA.alignment = align})) f =
  if DM.findWithDefault SbTBottom ptr f == SbTBottom
    then do
      ptr_meta <- CH.liftFuel freshName
      return $
        (phGUnit phI) `CH.catGraphNodeOO`
        (InsnInsn $ mkSbInit ptr_meta ptr (sizeOfType ty) maybeC align)
    else return $ phGUnit phI
...
sbNameTrackLattice =
  ...
  , CH.fact_join =
    \_ (CH.OldFact oldF) (CH.NewFact newF) -> joinSbNameTrackState oldF newF
  }
...
justJoinSbNameTrackState :: SbNameTrackState -> SbNameTrackState -> SbNameTrackState
