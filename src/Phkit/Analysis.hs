...
module Phkit.Analysis ( PhBwdAnalysis(..)
                      , PhBwdPass
                      , PhBwdTransfer
  ...
  where
    foldValue label k v (currChanged, currF) =
      let currV = DM.findWithDefault (CH.fact_bot per_name_fact_lattice) k currF
          (newChanged, newV) = CH.fact_join per_name_fact_lattice label (
            CH.OldFact currV) (CH.NewFact v)
...
fwdAnalysisWith lattice transfer = CH.FwdPass
...
bwdAnalysisWith lattice transfer = CH.BwdPass
...
aOnceLattice = CH.DataflowLattice
  { CH.fact_name = "Analyzed"
  , CH.fact_bot = False
  , CH.fact_join = joinFun
  } where
  joinFun _ (CH.OldFact old) (CH.NewFact new) = (CH.changeIf (
                                                    old  /= (old || new)),
                                                 old || new)
...
iSetAnalysisResultOf theModule =
  DS.unions $ concatMap (DM.elems . snd) $
    bwdAnalysisResultOf theModule $
    PhBwdAnalysis iSetLattice phInitWithNone iSetTransfer

iSetLattice :: CH.DataflowLattice InsnStringSet
iSetLattice = CH.DataflowLattice
  { CH.fact_name = "Instruction string set"
  , CH.fact_bot = DS.empty
  , CH.fact_join = add
  } where
  add _ (CH.OldFact old) (CH.NewFact new) = (changed, combined)
    where combined = DS.union new old
...
iSetTransfer = CH.mkBTransfer collect
  where
    ...
    collect (TermInsn (LGA.Do term) _) fbase = DS.fromList $ showTerm term :
      concatMap DS.toAscList (CH.mapElems fbase)
    collect (TermInsn (_ LGA.:= term) _) fbase = DS.fromList $ showTerm term :
      concatMap DS.toAscList (CH.mapElems fbase)
