...
ioStringOfAstModule modu =
  withContext $
  \ctx ->
    doOrThrow (withModuleFromAST ctx modu moduleLLVMAssembly) return
...
genBitcodeWithTransform fileIn fileOut moduleHandler =
  ...
  in
    doOrThrow moduleOrExceptT
    (\oldModule ->
      let newModule = moduleHandler oldModule in
      withContext
      (\context ->
        doOrThrow (
          ...
          (\cModule ->
            doOrThrow (
              writeBitcodeToFile (File fileOut) cModule) return
          )) return
      )
    )
...
withModuleFromPath path moduleHandler =
  ...
  (\bitcodeFilePath ->
    withContext
    (\c -> runExceptT (liftM moduleHandler (withModuleFromBitcode c
  ...
  buildModule [] [] (
    \bitcodeFilePath ->
      withContext (
        \c -> do
          aOrError <- runExceptT (
            withModuleFromBitcode c (LLVMModule.File bitcodeFilePath) (
...
runProcessInEnv :: FilePath -> [String] -> st ->
                   (st -> Maybe String -> (st, Maybe String)) -> IO st
runProcessInEnv filePath args initState stateTransformer = do
  (Just hin, Just hout, _, _) <- createProcess (
    proc filePath args){ std_in = CreatePipe, std_out = CreatePipe }
  invokeTransformer hin hout initState stateTransformer
  where invokeTransformer hin hout initS stateTrans = do
    ...
    where
      ...
      clangBuilder inp driver =
        withSystemTempFile ("base_" ++ takeFileName inp) $ \baseFname _ -> do
          let cOpts     = clangOpts ++ ["-emit-llvm", "-o" ,
                                        baseFname, "-c", inp]
          (_, _, _, p) <- createProcess $ proc driver cOpts
          rc <- waitForProcess p
          when (rc /= ExitSuccess) $ throwIO $ ClangFailed inputFilePath rc
          if null optOpts then parseFile baseFname
            else withSystemTempFile ("opt_" ++ takeFileName inp) $ \optFname _ -> do
              optify optOpts baseFname optFname
              parseFile optFname

buildAndExec :: Module -> [String] -> [String]
  ...
  withSystemTempFile ("base_" ++ takeFileName outputFilePath ++ ".bc") $
    \baseFilePath  _ -> do
      withContext (
        \context ->
          doOrThrow (
          withModuleFromAST context theModule
          (\cModule ->
            doOrThrow (
              writeBitcodeToFile (File baseFilePath) cModule) return
          )) return
        )
        ...
        \llcFilePath _ -> do
          ...
          case llcRet of
            ExitSuccess -> do
              clang <- findClang
              let cOpts = clangOpts ++ [
                    "-o", outputFilePath, llcFilePath]
              (_, _, _, p) <- createProcess $ proc clang cOpts
              rc <- waitForProcess p
              when (rc /= ExitSuccess) $ throwIO $
                ClangFailed outputFilePath rc
