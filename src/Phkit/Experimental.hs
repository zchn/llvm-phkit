module Phkit.Experimental
  ...
  , getBasicBlocks
  , getFunctionNames
  , listIndirectTransfer
  ) where

import Data.Maybe (mapMaybe)

import LLVM.General.AST
       (BasicBlock(..), Definition(GlobalDefinition), Instruction(Call),
        Global(Function), Operand(ConstantOperand, LocalReference), Module,
        Name, Named((:=), Do), Terminator(..), moduleDefinitions,
        moduleName)
...
getFunctionNames theModule = mapMaybe maybeGetName (moduleDefinitions theModule)
  where
...
isIndirectTransfer (NonTermSt (Call _ _ _ (Right (ConstantOperand _)) _ _ _)) =
  False
isIndirectTransfer (NonTermSt (Call _ _ _ (Right (LocalReference _ _)) _ _ _)) =
  True
...
getStatements (BasicBlock _ namedInstructions namedTerminator) =
  let unName (_ := a) = a
      unName (Do a) = a
  in map (NonTermSt . unName) namedInstructions ++
     [TermSt (unName namedTerminator)]
...
listIndirectTransfer theModule =
  map show $
  filter isIndirectTransfer $ concatMap getStatements $ getBasicBlocks theModule
